...
class RegisteredTrainerController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // Match by name + ID against the EC's pre-approved trainer whitelist,
        // same as the original signup flow.
        $whitelisted = TrainerWhitelist::whereRaw('LOWER(first_name) = LOWER(?)', [$data['first_name']])
        ...
        if (! $whitelisted) {
            throw ValidationException::withMessages([
                'general' => 'No matching approved trainer found for the name and ID you entered, or the account has already been registered. Please contact the Extension Coordinator.',
            ]);
        }
        ...
        return redirect()->route('login')->with('success', 'Trainer account created! Your ID is ' . $whitelisted->id_number . '. You can now log in.');
    }
}
