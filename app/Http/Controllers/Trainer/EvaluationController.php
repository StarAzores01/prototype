...
class EvaluationController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($viewTrainingId) {
            $training = Training::where('id', $viewTrainingId)->where('trainer_id', $trainerId)->first();
    ...
    private function mainView(int $trainerId)
    {
        ...
        $avgRatingRaw = Evaluation::whereHas('training', fn ($t) => $t->where('trainer_id', $trainerId))
            ->whereNotNull('rating')
            ->avg('rating');
        $avgRating = $avgRatingRaw !== null ? round((float) $avgRatingRaw, 1) : null;

        $submitted = Evaluation::whereHas('training', fn ($t) => $t->where('trainer_id', $trainerId))
            ->where('status', 'Submitted')->count();

        $pending = Evaluation::whereHas('training', fn ($t) => $t->where('trainer_id', $trainerId))
            ->where('status', 'Pending')->count();

        $evalData = Training::where('trainer_id', $trainerId)
