...
if (isLoggedIn()) redirect(BASE_URL . '/beneficiary/trainings.php');
    ...
    if (empty($errors)) {
        $check = $pdo->prepare(
            'SELECT id FROM participants
             WHERE LOWER(full_name)=LOWER(?) AND id_number=? AND (beneficiary_id IS NULL OR beneficiary_id=0)
             LIMIT 1'
        );
        $check->execute([$old['first_name'].' '.$old['last_name'], $old['id_number']]);
        $participant = $check->fetch();

        if (!$participant) {
            $errors['id_number'] = 'No matching participant record found, or an account already exists for this ID. Please contact the Extension Coordinator.';
        } else {
            $dup = $pdo->prepare('SELECT id FROM beneficiaries WHERE email=? OR username=?');
            $dup->execute([$old['email'], $old['username']]);
            if ($dup->fetch()) {
                $errors['email'] = 'An account with this email or username already exists.';
            } else {
                $hash = password_hash($password, PASSWORD_BCRYPT, ['cost' => 12]);
                $pdo->prepare('INSERT INTO beneficiaries (username,first_name,last_name,email,phone,address,age,sex,password_hash) VALUES (?,?,?,?,?,?,?,?,?)')
                    ->execute([$old['username'],$old['first_name'],$old['last_name'],$old['email'],$old['phone'],$old['address'],$old['age']?:null,$old['sex']?:null,$hash]);
                $newId = $pdo->lastInsertId();
                $pdo->prepare('UPDATE participants SET beneficiary_id=? WHERE LOWER(full_name)=LOWER(?) AND id_number=?')
                    ->execute([$newId, $old['first_name'].' '.$old['last_name'], $old['id_number']]);
  ...
  <div class="auth-card-body">
    ...
    <form method="POST" novalidate>
      ...
      <div class="form-group">
        ...
        <input type="text" name="id_number" class="form-control" value="<?= e($old['id_number'] ?? '') ?>" placeholder="e.g. 2024-00001" required/>
        <?php if (!empty($errors['id_number'])): ?><div class="form-error"><?= e($errors['id_number']) ?></div><?php endif; ?>
      </div>

      <div class="form-group">
        <label class="form-label">Email Address *</label>
        <input type="email" name="email" class="form-control" value="<?= e($old['email'] ?? '') ?>" placeholder="juan@email.com" required/>
        <?php if (!empty($errors['email'])): ?><div class="form-error"><?= e($errors['email']) ?></div><?php endif; ?>
      </div>

      <div class="form-group">
        <label class="form-label">Phone Number</label>
        <input type="text" name="phone" class="form-control" value="<?= e($old['phone'] ?? '') ?>" placeholder="0912 345 6789" maxlength="13" oninput="fmtPhone(this)"/>
      </div>

      <div class="form-group">
        <label class="form-label">Address</label>
        <input type="text" name="address" class="form-control" value="<?= e($old['address'] ?? '') ?>" placeholder="Brgy. San Antonio, Lucban, Quezon"/>
      </div>

      <div class="form-row">
        <div class="form-group">
          <label class="form-label">Age</label>
          <input type="number" name="age" class="form-control" value="<?= e($old['age'] ?? '') ?>" placeholder="25" min="1" max="120"/>
        </div>
        <div class="form-group">
