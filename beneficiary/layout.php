...
$notifList  = [];
try {
    $ns = $pdo->prepare('SELECT * FROM notifications WHERE is_read=0 AND role="beneficiary" AND user_id=? ORDER BY created_at DESC LIMIT 8');
...
<head>
  ...
  <title>PAThrive – <?= ucfirst($activePage ?? 'Portal') ?></title>
...
<header class="topbar">
  ...
  <div class="topbar-center">
    <div class="topbar-title"><strong>Participant Portal</strong> &nbsp;·&nbsp; Extension Training Management System</div>
  </div>
  <div class="topbar-right">
    ...
    <div class="topbar-icon-btn notif-btn" title="Notifications" onclick="toggleNotifDropdown()" style="position:relative">
      ...
      <div id="notifDropdown" style="display:none;position:absolute;top:calc(100% + 8px);right:0;width:320px;background:#fff;border-radius:12px;box-shadow:0 8px 32px rgba(0,0,0,.15);border:1px solid var(--gray-200);z-index:1000;overflow:hidden">
        <div style="padding:12px 16px;border-bottom:1px solid var(--gray-100);display:flex;justify-content:space-between;align-items:center">
          ...
          <?php if ($notifCount > 0): ?>
          <a href="<?= BASE_URL ?>/beneficiary/notifications.php?mark_all_read=1" style="font-size:11px;color:var(--blue-primary)">Mark all read</a>
          <?php endif; ?>
        </div>
    ...
    <div class="profile-btn" onclick="toggleDropdown()" role="button">
      ...
      <div class="profile-dropdown" id="profileDropdown">
        <a href="<?= BASE_URL ?>/beneficiary/profile.php">&#128100; My Profile</a>
        <hr>
        <a href="<?= BASE_URL ?>/beneficiary/logout.php" style="color:var(--red)">&#10148; Logout</a>
      </div>
    </div>
  </div>
</header>
...
<aside class="sidebar">
  <div class="sidebar-section">
    ...
    <a href="<?= BASE_URL ?>/beneficiary/<?= $key ?>.php" class="sidebar-item <?= ($activePage === $key) ? 'active' : '' ?>">
