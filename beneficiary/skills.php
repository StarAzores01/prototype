...
$pdo->exec("CREATE TABLE IF NOT EXISTS skills_responses (
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'submit') {
    ...
    $already = $pdo->prepare('SELECT id FROM skills_responses WHERE form_id=? AND beneficiary_id=?');
    $already->execute([$formId, $bid]);
    if ($already->fetch()) {
        setFlash('error', 'You have already submitted this survey and it can no longer be edited.');
        redirect(BASE_URL.'/beneficiary/skills.php');
    }

    $pdo->prepare('INSERT INTO skills_responses (form_id,training_id,beneficiary_id,responses)
                   VALUES (?,?,?,?)')
    ...
    setFlash('success','Skills survey submitted. Thank you!');
    redirect(BASE_URL.'/beneficiary/skills.php');
}

// ── Load skills forms sent to this beneficiary ─────────────────────────────
try {
    $forms = $pdo->prepare(
        'SELECT sf.*, t.title AS training_title, t.date_start,
                sr.id AS response_id, sr.responses, sr.submitted_at AS responded_at
         ...
         WHERE p.beneficiary_id=? AND sf.sent_at IS NOT NULL
...
<div class="page-header">
  ...
  <?php if ($viewForm): ?>
  <a href="<?= BASE_URL ?>/beneficiary/skills.php" class="btn btn-outline">&#8592; Back</a>
  <?php endif; ?>
</div>
...
<div class="card">
  ...
  <div class="card-body">
    ...
    <div style="background:#F0FDF4;border:1.5px solid #BBF7D0;border-radius:10px;padding:12px 16px;margin-bottom:20px;display:flex;align-items:center;gap:10px;font-size:13px;color:#166534">
      &#9989; <strong>You have already submitted this survey.</strong> Responses can no longer be edited.
    </div>
    <?php
    ...
    <div style="display:flex;justify-content:flex-end;margin-top:8px">
      <a href="<?= BASE_URL ?>/beneficiary/skills.php" class="btn btn-outline">&#8592; Back to List</a>
    </div>
    ...
    <form method="POST">
      ...
      <div style="display:flex;justify-content:flex-end;gap:10px;margin-top:8px">
        <a href="<?= BASE_URL ?>/beneficiary/skills.php" class="btn btn-outline">Cancel</a>
...
<div class="card">
  <div class="card-body" style="text-align:center;padding:60px 24px">
    &#128200;
    <div style="font-size:15px;font-weight:700;color:var(--navy);margin:12px 0 6px">No skills surveys yet</div>
    <p style="font-size:13px;color:var(--gray-400)">Your trainer will send a skills utilization survey after your training.</p>
  </div>
</div>
<?php else: ?>
<div style="display:flex;flex-direction:column;gap:14px">
  ...
  <div class="card" style="border-left:4px solid <?= $f['response_id'] ? 'var(--green)' : 'var(--blue-primary)' ?>">
    <div class="card-body" style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:12px">
      ...
      <?php else: ?>
      <a href="?form=<?= $f['id'] ?>" class="btn btn-primary btn-sm">&#128221; Answer Survey</a>
      <?php endif; ?>
    </div>
  </div>
  <?php endforeach; ?>
