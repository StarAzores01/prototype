...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'update_profile') {
        ...
        redirect(BASE_URL . '/evaluator/profile.php');
    }
      ...
      <div class="card-body">
        <form method="POST">
          ...
          <div class="form-row">
            ...
            <div class="form-group"><label class="form-label">Evaluator ID <span style="font-size:10px;color:var(--gray-400)">(system-assigned)</span></label><input type="text" class="form-control field-readonly" value="<?= e($user['id_number']??'') ?>" readonly/></div>
          </div>
          <div class="form-row">
            ...
            <div class="form-group"><label class="form-label">Department</label><input type="text" class="form-control field-readonly" value="<?= e($user['position']??'') ?>" readonly/></div>
          </div>
    ...
    <div class="card">
      ...
      <div class="card-body">
        <form method="POST">
          ...
          <div class="form-group"><label class="form-label">Current Password</label><div class="pwd-wrap"><input type="password" name="current_password" id="p1" class="form-control" required/><span class="pwd-eye" onclick="tp('p1',this)">&#128065;</span></div></div>
          <div class="form-row">
            <div class="form-group"><label class="form-label">New Password</label><div class="pwd-wrap"><input type="password" name="new_password" id="p2" class="form-control" required/><span class="pwd-eye" onclick="tp('p2',this)">&#128065;</span></div></div>
            <div class="form-group"><label class="form-label">Confirm Password</label><div class="pwd-wrap"><input type="password" name="confirm_password" id="p3" class="form-control" required/><span class="pwd-eye" onclick="tp('p3',this)">&#128065;</span></div></div>
          </div>
  ...
  <div class="dash-side">
    <div class="card">
      <div class="card-body" style="text-align:center;padding:32px 24px">
        ...
        <div style="margin-top:16px;display:flex;flex-direction:column;gap:10px;text-align:left">
          <?php foreach ([['fa-envelope',$user['email']??'—'],['fa-id-card',$user['id_number']??'—'],['fa-building',$user['position']??'—']] as [$ic,$vl]): ?>
          <div style="display:flex;align-items:center;gap:10px;font-size:13px;color:var(--gray-700)"><i class="fas <?= $ic ?>" style="width:16px;color:#8B5CF6"></i><?= e($vl) ?></div>
          <?php endforeach; ?>
        </div>
      </div>
    </div>
    <div class="card">
      ...
      <div class="card-body" style="display:flex;flex-direction:column;gap:12px">
        <?php foreach ([['Role','Evaluator'],['Status',$user['is_active']?'Active':'Inactive'],['Member Since',isset($user['created_at'])?date('M d, Y',strtotime($user['created_at'])):'—']] as [$lb,$vl]): ?>
...
function tp(id,icon){const f=document.getElementById(id);f.type=f.type==='password'?'text':'password';icon.textContent=f.type==='password'?'👁':'🙈';}
</script>
