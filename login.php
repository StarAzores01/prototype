...
if (isLoggedIn()) {
    ...
    if ($role === 'extension_coordinator') redirect(BASE_URL . '/ec/dashboard.php');
    if ($role === 'trainer')               redirect(BASE_URL . '/trainer/dashboard.php');
    if ($role === 'beneficiary')           redirect(BASE_URL . '/beneficiary/home.php');
    if ($role === 'evaluator')             redirect(BASE_URL . '/evaluator/dashboard.php');
    redirect(BASE_URL . '/eclandingpage.php');
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($login) || empty($password)) {
        ...
        // Also check beneficiaries table
        if (!$user) {
            $stmt2 = $pdo->prepare('SELECT *, "beneficiary" AS role FROM beneficiaries WHERE (email=? OR username=?) AND is_active=1');
        ...
        if ($user && password_verify($password, $user['password_hash'])) {
            ...
            // Update last_login only for users table
            if ($user['role'] !== 'beneficiary') {
                $pdo->prepare('UPDATE users SET last_login=NOW() WHERE id=?')->execute([$user['id']]);
            }
            if ($user['role'] === 'extension_coordinator') redirect(BASE_URL . '/ec/dashboard.php');
            if ($user['role'] === 'trainer')               redirect(BASE_URL . '/trainer/dashboard.php');
            if ($user['role'] === 'beneficiary')           redirect(BASE_URL . '/beneficiary/home.php');
            if ($user['role'] === 'evaluator')             redirect(BASE_URL . '/evaluator/dashboard.php');
            redirect(BASE_URL . '/eclandingpage.php');

        } elseif ($user && !$user['is_active']) {
            $error = 'Your account has been deactivated. Contact the Extension Coordinator.';
        } else {
...
<head>
  ...
  <style>
    ...
    .li-ti-grn { background: #D1FAE5; color: #065F46; }
    .li-ti-bl  { background: #DBEAFE; color: #1E3A8A; }
    .li-ti-nv  { background: #E0E7FF; color: #3730A3; }
  ...
  <div class="auth-card">
    ...
    <div class="auth-card-header">
      ...
      <div class="brand-logo" style="margin:0 auto 14px">
        <img src="<?= BASE_URL ?>/imgs/logofinalpt.png" alt="CIT"
             style="width:44px;height:44px;object-fit:contain"
             onerror="this.style.display='none'"/>
      </div>
      <h1>Welcome Back</h1>
      <p>Log in to PAThrive &nbsp;·&nbsp; CIT-SLSU</p>
    </div>

    <!-- Card body -->
    <div class="auth-card-body">

      <!-- Alerts — original PHP variables completely untouched -->
      <?php if ($msg): ?>
      <div class="alert alert-info">&#8505; <?= $msg ?></div>
      <?php endif; ?>
      <?php if ($error): ?>
      <div class="alert alert-danger">&#9888; <?= e($error) ?></div>
      <?php endif; ?>

      <!-- Form — method, all name attrs, novalidate all untouched -->
      <form method="POST" novalidate>
        ...
        <div class="form-group">
          ...
          <div class="password-wrap">
            <input
              ...
              required
            />
            <!-- id="eyeIcon" and togglePwd() kept exactly as original -->
        ...
        <!-- Forgot password — original href untouched -->
        <a href="<?= BASE_URL ?>/ecrecovery.php" class="li-forgot">Forgot password?</a>
    ...
    <!-- Footer — original href untouched -->
    <div class="auth-footer">
      Don't have an account? <a href="<?= BASE_URL ?>/choose-role.php">Sign up here</a>
    </div>

  </div><!-- /.auth-card -->

<!-- Original JavaScript — completely untouched -->
<script>
function togglePwd() {
