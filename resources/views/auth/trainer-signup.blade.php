...
<div class="auth-card" style="max-width:520px">
  ...
  <div class="auth-card-body">
    ...
    <div style="text-align:center;padding:16px 0">
      ...
      <a href="{{ route('choose-role') }}" class="btn btn-outline" style="margin-right:8px">Back</a>
      <a href="{{ route('login') }}" class="btn btn-primary">Log In Instead</a>
    </div>
    ...
    <div class="alert alert-info" style="margin-bottom:16px">
      <i class="fas fa-circle-info"></i> Only pre-approved CIT faculty members can register as Project Leaders.
    </div>

    <form method="POST" action="{{ route('trainer.signup') }}" novalidate>
      @csrf
      <div class="form-row">
        <div class="form-group">
          <label class="form-label">First Name *</label>
          <input type="text" name="first_name" class="form-control" value="{{ old('first_name') }}" placeholder="As on the approved list" required/>
          @error('first_name')<div class="form-error">{{ $message }}</div>@enderror
        </div>
        <div class="form-group">
          <label class="form-label">Last Name *</label>
          <input type="text" name="last_name" class="form-control" value="{{ old('last_name') }}" placeholder="As on the approved list" required/>
      ...
      <div class="form-group">
        ...
        <input type="text" name="id_number" class="form-control" value="{{ old('id_number') }}" placeholder="PL-YYYY-XXXX" required/>
        @error('id_number')<div class="form-error">{{ $message }}</div>@enderror
      </div>
      <div class="form-group">
        <label class="form-label">Username *</label>
        <input type="text" name="username" class="form-control" value="{{ old('username') }}" placeholder="e.g. jcampita" required/>
        @error('username')<div class="form-error">{{ $message }}</div>@enderror
      </div>
      <div class="form-group">
        <label class="form-label">Email Address *</label>
        <input type="email" name="email" class="form-control" value="{{ old('email') }}" placeholder="email@slsu.edu.ph" required/>
      ...
      <div class="form-group" style="display:flex;align-items:flex-start;gap:10px">
        ...
        <label for="agree" style="font-size:12.5px;color:var(--gray-600);cursor:pointer">
          I agree to the <a href="{{ route('terms') }}" style="color:var(--blue-primary);font-weight:600">Terms of Use</a> and <a href="{{ route('privacy') }}" style="color:var(--blue-primary);font-weight:600">Privacy Policy</a>
        </label>
      </div>
      ...
      <button type="submit" class="btn btn-primary" style="width:100%;padding:11px;font-size:14px;justify-content:center">
        <i class="fas fa-plus"></i> Create Trainer Account
      </button>
    </form>
    @endif
  </div>
