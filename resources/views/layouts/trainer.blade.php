<?php
// Static nav config — same structure as the original $nav array in trainer/layout.php.
$nav = [
    'dashboard'    => ['icon' => 'fa-gauge-high',   'label' => 'Dashboard'],
    'programs'     => ['icon' => 'fa-diagram-project', 'label' => 'Programs'],
    'trainings'    => ['icon' => 'fa-book',         'label' => 'My Activities'],
    'participants' => ['icon' => 'fa-users',        'label' => 'Participants'],
    'attendance'   => ['icon' => 'fa-clipboard-list','label' => 'Attendance'],
    'documents'    => ['icon' => 'fa-folder-open',  'label' => 'Documents'],
    // Evaluations and Skills Utilization are consolidated under this one hub
    // page — those 2 sub-pages still exist unchanged, reached from the hub.
    'evaluation'   => ['icon' => 'fa-square-check', 'label' => 'Evaluation'],
    'activity'     => ['icon' => 'fa-file-lines',   'label' => 'Activity Report'],
];
$activePage = $activePage ?? 'dashboard';
// $activePage is the 'trainings' route/key on purpose (unchanged) — its
// display label is "My Activities" now, so the <title> tag needs its own
...
<head>
  ...
  <title>PAThrive – {{ $activePageTitle }} (Project Leader)</title>
...
<header class="topbar">
  ...
  <div class="topbar-center">
    <div class="topbar-title"><strong>Project Leader Portal</strong> &nbsp;&middot;&nbsp; Extension Training Management System</div>
  </div>
  <div class="topbar-right">
    ...
    <div class="topbar-icon-btn notif-btn" title="Notifications" onclick="toggleNotifDropdown()" style="position:relative">
      ...
      <div class="notif-dropdown" id="notifDropdown" style="display:none;position:absolute;top:calc(100% + 8px);right:0;width:320px;background:var(--surface);border-radius:12px;box-shadow:0 8px 32px rgba(0,0,0,.15);border:1px solid var(--gray-200);z-index:1000;overflow:hidden">
        <div style="padding:12px 16px;border-bottom:1px solid var(--gray-100);display:flex;justify-content:space-between;align-items:center">
          ...
          @if($notifCount > 0)
          <a href="{{ route('trainer.notifications', ['mark_all_read' => 1]) }}" style="font-size:11px;color:var(--blue-primary)">Mark all read</a>
          @endif
        </div>
        <div style="max-height:320px;overflow-y:auto">
          @forelse($notifList as $n)
          <a href="{{ route('trainer.notifications.open', $n->id) }}" style="display:block;padding:12px 16px;border-bottom:1px solid var(--gray-100);text-decoration:none;background:rgba(59,130,246,.12)">
    ...
    <div class="profile-btn" onclick="toggleDropdown()" role="button" aria-haspopup="true" aria-expanded="false">
      ...
      <div class="profile-dropdown" id="profileDropdown">
        <a href="{{ route('trainer.profile') }}"><i class="fas fa-user"></i> My Profile</a>
        <hr>
...
<aside class="sidebar" id="mainSidebar">
  <div class="sidebar-section">
    ...
    @foreach(['dashboard','programs','trainings'] as $key)
    <a href="{{ route('trainer.' . $key) }}" class="sidebar-item {{ $activePage === $key ? 'active' : '' }}">
  ...
  <div class="sidebar-section">
    ...
    @foreach(['participants','attendance','documents'] as $key)
    <a href="{{ route('trainer.' . $key) }}" class="sidebar-item {{ $activePage === $key ? 'active' : '' }}">
      <span><i class="fas {{ $nav[$key]['icon'] }}"></i></span><span>{{ $nav[$key]['label'] }}</span>
    </a>
    @endforeach
  ...
  <div class="sidebar-section">
    ...
    @foreach(['evaluation','activity'] as $key)
    @php
      $isActive = $key === 'evaluation'
        ? in_array($activePage, ['evaluation', 'evaluations', 'skills'], true)
        : $activePage === $key;
    @endphp
    <a href="{{ route('trainer.' . $key) }}" class="sidebar-item {{ $isActive ? 'active' : '' }}">
      <span><i class="fas {{ $nav[$key]['icon'] }}"></i></span><span>{{ $nav[$key]['label'] }}</span>
    </a>
    @endforeach
  </div>
