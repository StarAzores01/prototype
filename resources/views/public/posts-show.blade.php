...
<head>
  ...
  <title>PAThrive – {{ $post->title }}</title>
  ...
  <style>
    ...
    .post-meta-row { display:flex;align-items:center;gap:18px;flex-wrap:wrap;color:rgba(255,255,255,.55);font-size:13px;margin-top:18px; }
    .post-meta-row span { display:inline-flex;align-items:center;gap:6px; }
    .post-article { max-width:760px;margin:-28px auto 0;padding:0 24px 80px;position:relative;z-index:1; }
    .post-article-card { background:#fff;border-radius:16px;box-shadow:0 12px 48px rgba(9,24,47,.14);overflow:hidden; }
    .post-article-img { width:100%;height:320px;object-fit:cover;display:block; }
    .post-article-body { padding:36px 40px; }
    .post-article-body p { font-size:15.5px;line-height:1.85;color:#334155;margin:0 0 16px;white-space:pre-line; }
...
<div class="page-hero">
  <div style="max-width:760px;margin:0 auto;padding:0 24px">
    <div class="eyebrow"><i class="fas fa-bullhorn"></i> Announcement</div>
    <h1 style="font-size:clamp(26px,4vw,38px);font-weight:800;color:#fff;margin-bottom:0;line-height:1.25">{{ $post->title }}</h1>
    <div class="post-meta-row">
      <span><i class="fas fa-calendar"></i> {{ ($post->published_at ?? $post->created_at)?->format('F j, Y') }}</span>
      <span><i class="fas fa-user"></i> {{ $post->author->full_name ?? 'Extension Coordinator' }}</span>
      <span><i class="fas fa-eye"></i> {{ number_format($post->views) }} {{ Str::plural('view', $post->views) }}</span>
    </div>
  </div>
</div>
...
<div class="post-article">
  <div class="post-article-card">
    @if($post->image)
      <img src="{{ $post->image }}" alt="{{ $post->title }}" class="post-article-img"/>
    @endif
  ...
  <div style="margin-top:24px">
    <a href="{{ route('public.posts.index') }}" style="color:#1A56DB;font-size:13.5px;font-weight:600;text-decoration:none"><i class="fas fa-arrow-left"></i> Back</a>
  </div>
</div>
