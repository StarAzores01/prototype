...
@php
  ...
  function docDateLabel($doc, $today, $yesterday) {
    $ts = $doc->created_at;
...
<div class="card" style="margin-bottom:24px">
  <div class="card-header">
    <div><div class="card-title">My Documents</div><div class="card-subtitle">{{ $myDocs->count() }} file{{ $myDocs->count() === 1 ? '' : 's' }}</div></div>
  </div>
  <div class="table-wrap">
    ...
    @else
    @php $myGrouped = $myDocs->groupBy(fn($d) => docDateLabel($d, $today, $yesterday)); @endphp
    @foreach($myGrouped as $dateLabel => $group)
      <div class="doc-date-group-header">
        <i class="fas fa-calendar-alt" style="margin-right:6px;opacity:.6"></i>{{ $dateLabel }}
        <span style="font-weight:400;margin-left:8px;opacity:.7">({{ $group->count() }})</span>
      </div>
      <table class="data-table">
        <thead><tr><th>Document</th><th>Type</th><th class="doc-table-linked-to">Activity</th><th>Visibility</th><th>Time</th><th>Actions</th></tr></thead>
        <tbody>
        @foreach($group as $d)
          ...
          <tr>
            <td>
              <div style="display:flex;align-items:center;gap:8px;min-width:0">
                @if($d->isLink())
                  <span style="width:32px;height:32px;border-radius:8px;background:var(--blue-soft);color:var(--blue-primary);display:flex;align-items:center;justify-content:center;flex-shrink:0;font-size:14px"><i class="{{ $linkIcon[$d->link_type] ?? 'fa-solid fa-link' }}"></i></span>
                @else
                  @php $ico = match(strtolower($d->file_type ?? '')) { 'pdf' => ['fa-file-pdf','#FEE2E2','#EF4444'], 'doc','docx' => ['fa-file-word','#DBEAFE','#1A56DB'], 'xls','xlsx' => ['fa-file-excel','#D1FAE5','#10B981'], 'jpg','jpeg','png','gif','webp' => ['fa-file-image','#E0E7FF','#6366F1'], 'mp4' => ['fa-file-video','#FEF3C7','#F59E0B'], default => ['fa-file','#F1F5F9','#64748B'] }; @endphp
                  <span style="width:32px;height:32px;border-radius:8px;background:{{ $ico[1] }};color:{{ $ico[2] }};display:flex;align-items:center;justify-content:center;flex-shrink:0;font-size:14px"><i class="fas {{ $ico[0] }}"></i></span>
                @endif
                <span style="font-size:13px;font-weight:600;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:200px" title="{{ $d->original_name }}">{{ $d->original_name }}</span>
              </div>
            </td>
            ...
            <td class="doc-table-linked-to" style="font-size:12px;color:var(--gray-600)">{{ Str::limit($d->training->title ?? 'General', 28) }}</td>
            <td>
            ...
            <td>
              <div class="action-btns">
                ...
                @endif
                <button class="btn btn-sm btn-danger" onclick="if(confirm('Delete this document?')){document.getElementById('del{{ $d->id }}').submit()}" title="Delete"><i class="fas fa-trash"></i></button>
                <form id="del{{ $d->id }}" method="POST" action="{{ route('trainer.documents.store') }}" style="display:none">@csrf<input type="hidden" name="action" value="delete"/><input type="hidden" name="doc_id" value="{{ $d->id }}"/></form>
              </div>
            </td>
          </tr>
        @endforeach
...
<div class="card">
  ...
  <div class="table-wrap">
    ...
    @else
    @php $sharedGrouped = $sharedDocs->groupBy(fn($d) => docDateLabel($d, $today, $yesterday)); @endphp
    @foreach($sharedGrouped as $dateLabel => $group)
      <div class="doc-date-group-header">
        <i class="fas fa-calendar-alt" style="margin-right:6px;opacity:.6"></i>{{ $dateLabel }}
        <span style="font-weight:400;margin-left:8px;opacity:.7">({{ $group->count() }})</span>
      </div>
      <table class="data-table">
        <thead><tr><th>Document</th><th>Type</th><th class="doc-table-linked-to">Activity</th><th class="doc-table-uploaded-by">Shared By</th><th>Time</th><th>Actions</th></tr></thead>
        <tbody>
        @foreach($group as $d)
          <tr>
            <td>
            ...
            <td><span style="font-size:11px;font-weight:700;text-transform:uppercase;color:var(--gray-500)">{{ $d->isLink() ? $d->link_type : $d->file_type }}</span></td>
            <td class="doc-table-linked-to" style="font-size:12px;color:var(--gray-600)">{{ Str::limit($d->training->title ?? 'General', 28) }}</td>
            <td class="doc-table-uploaded-by" style="font-size:12px;color:var(--gray-600)">{{ $d->uploader?->full_name ?? '—' }}</td>
            <td style="font-size:12px;color:var(--gray-400);white-space:nowrap">{{ $d->created_at?->format('g:i A') ?? '—' }}</td>
            <td>
              <div class="action-btns">
                @if($d->isLink())
